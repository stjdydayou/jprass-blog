...
(function ($) {
    ...
    $.validator.config({
        ...
        rules: {
            ...
            tel: [/^(?:(?:0\d{2,3}[- ]?[1-9]\d{6,7})|(?:[48]00[- ]?[1-9]\d{6}))$/, "{0}格式不正确"],  //办公或家庭电话
            mobile: [/^1[3-9]\d{9}$/, "{0}格式不正确"],  //移动电话
            email: [/^(?:[a-z0-9]+[_\-+.]?)*[a-z0-9]+@(?:([a-z0-9]+-?)*[a-z0-9]+.)+([a-z]{2,})+$/i, "{0}格式不正确"],
            qq: [/^[1-9]\d{4,}$/,"{0}格式不正确"],
            date: [/^\d{4}-\d{1,2}-\d{1,2}$/, "请输入正确的{0}日期,例:yyyy-mm-dd"],
            time: [/^([01]\d|2[0-3])(:[0-5]\d){1,2}$/, "请输入正确的{0}时间,例:14:30或14:30:00"],
            ID_card: [/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/, "请输入正确的身份证号码"],
            url: [/^(https?|ftp):\/\/[^\s]*$/i, "{0}格式不正确"],
            postcode: [/^[1-9]\d{5}$/, "{0}格式不正确"], //邮政编码
            chinese: [/^[\u0391-\uFFE5]+$/, "请输入中文"],
            username: [/^\w{3,12}$/, "请输入3-12位数字、字母、下划线"], //用户名
            password: [/^[0-9a-zA-Z]{6,16}$/, "{0}由6-16位数字、字母组成"], //密码
            //可接受的后缀名
